...
test("OpenAI uses one stateless native web-search call and extracts exposed queries and sources",async()=>{
  ...
  assert.equal(request.url,"https://api.openai.com/v1/responses");assert.deepEqual(request.body.tools,[{type:"web_search",search_context_size:"low"}]);assert.equal(request.body.tool_choice,"required");assert.equal(request.body.max_tool_calls,MAX_NATIVE_SEARCHES);assert.equal(request.body.max_output_tokens,NATIVE_MAX_OUTPUT_TOKENS);assert.equal(request.body.store,false);assert.equal(JSON.stringify(request.body).includes("openai-secret"),false);assert.equal(request.options.headers.Authorization,"Bearer openai-secret");assert.deepEqual(result.queries,["best seo software","seo tools pricing"]);assert.equal(result.sources.length,2);
});
