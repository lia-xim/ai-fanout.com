...
import { MAX_NATIVE_SEARCHES, NATIVE_MAX_OUTPUT_TOKENS, NATIVE_RESERVE_MICRO_EUR } from "../src/server/fanout/native-contracts.mjs";
...
test("OpenAI uses one stateless native web-search call and extracts exposed queries and sources",async()=>{
  ...
  assert.equal(request.url,"https://api.openai.com/v1/responses");assert.deepEqual(request.body.tools,[{type:"web_search",search_context_size:"low"}]);assert.equal(request.body.tool_choice,"required");assert.equal(request.body.max_tool_calls,MAX_NATIVE_SEARCHES);assert.equal(request.body.max_output_tokens,NATIVE_MAX_OUTPUT_TOKENS);assert.equal(request.body.store,false);assert.equal(JSON.stringify(request.body).includes("openai-secret"),false);assert.equal(request.options.headers.Authorization,"Bearer openai-secret");assert.deepEqual(result.queries,["best seo software","seo tools pricing"]);assert.equal(result.sources.length,2);assert.equal(result.searchActions[0].sourceScope,"search_action");assert.deepEqual(result.searchActions[0].sources,[{url:"https://example.com/a",title:"A"}]);assert.equal(result.usage.estimatedCostUsd,0.01004);
});
...
test("Gemini uses native Google Search and extracts executed queries and citations",async()=>{
  ...
  assert.equal(request.url,"https://generativelanguage.googleapis.com/v1beta/interactions");assert.deepEqual(request.body.tools,[{type:"google_search"}]);assert.equal(request.body.generation_config.max_output_tokens,NATIVE_MAX_OUTPUT_TOKENS);assert.equal(request.body.store,false);assert.equal(JSON.stringify(request.body).includes("gemini-secret"),false);assert.equal(request.options.headers["x-goog-api-key"],"gemini-secret");assert.deepEqual(result.queries,["beste seo tools 2026","seo software vergleich"]);assert.equal(result.sources[0].url,"https://example.org/source");assert.equal(result.searchActions[0].sourceScope,"not_exposed");assert.equal(result.usage.estimatedCostUsdMaximum,0.028082);
});
...
test("OpenAI keeps an exposed search action even when only the unused final answer is incomplete",async()=>{
  const provider=new OpenAINativeProvider({apiKey:"x",fetchImpl:async()=>({ok:true,json:async()=>({status:"incomplete",output:[{type:"web_search_call",action:{type:"search",queries:["seo tools for small business"],sources:[]}}],usage:{input_tokens:10,output_tokens:5}})})});
  const result=await provider.observe(input);assert.deepEqual(result.queries,["seo tools for small business"]);assert.equal(result.providerResponseStatus,"incomplete");
});
...
test("native service is strict, CAPTCHA-first, separately bucketed, and stores no raw input",async()=>{
  ...
  const providers={openai:{observe:async()=>({queries:["seo tools"],sources:[],searchActions:[],searchActionCount:1,usage:{inputTokens:1,outputTokens:2,searchActionCount:1,searchQueryCount:1,estimatedCostUsd:.01,pricingBasis:"test",pricingCheckedAt:"2026-08-27"},model:"gpt-5.6-luna",provider:"openai",inputTokens:1,outputTokens:2,latencyMs:3})}};
  ...
  const result=await service({body:input,remoteIp:"1.2.3.4"});assert.equal(result.evidenceStatus,"provider_exposed_native_search");assert.deepEqual(result.queries,["seo tools"]);assert.equal(result.sourceEvidenceScope,"search_action_when_exposed");assert.equal(result.usage.inputTokens,1);assert.equal(JSON.stringify([...ledger.records.values()]).includes(input.keyword),false);
});
