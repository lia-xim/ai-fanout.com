...
One live result supports query-level selection, selection-only JSON or keyword CSV export, and a browser-local handoff to the established SEO Fanout and Contextter workflows. Saved runs can be compared by provider or by date. Source domains remain attached only at the scope actually returned by the provider; exports never invent a query-to-source relation.

## Evidence system

The initial four owner-run OpenAI observations are published as versioned JSON, normalized CSV and a JSON Schema. A separate 12-topic monthly observation plan is explicitly marked `planned_not_results`. Public Gemini evidence remains excluded until retention and publication rights are proven.
