...
import { NATIVE_METHOD_VERSION, NATIVE_TOOL_VERSION, validateNativeRequest } from "./native-contracts.mjs";
...
export function createNativeFanoutService({ ledger, providers, captchaVerifier, bucketSalt, now = () => new Date() }) {
  return async ({ body, remoteIp = "unknown" }) => {
    ...
    await ledger.reserve({ bucketHash, reservationId, questionHash: hash(bucketSalt, `${input.keyword}|${input.provider}|${input.language}|${input.country}`), model: input.provider, plannerVersion: NATIVE_TOOL_VERSION });
    try {
